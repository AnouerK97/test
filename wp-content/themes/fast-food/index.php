...
if ( have_posts() ) {
	...
	while ( have_posts() ) {
		...
		$content_final_html  = '<p>' . get_the_excerpt() . '</p>';
	...
	if ( is_search() ) { ?>
		<article class="btNoSearchResults boldSection gutter bottomSemiSpaced topSemiSpaced ">
			<div class="port">
			<?php echo boldthemes_get_heading_html( '', esc_html__( 'We are sorry, no results for: ', 'fast-food' ) . get_search_query(), '<a href="' . site_url() . '">' . esc_html__( 'Back to homepage', 'fast-food' ) . '</a>', 'medium', '', '', '' ) ?>
			</div>
		</article>
	<?php }
}
