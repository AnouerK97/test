...
if ( have_posts() ) {
							...
							<div class="rowItemContent">
												...
												<div class="btSlideGutter gutter">
													<div class="btSlidePane port">
														<div class="col-sm-9 rowItem <?php echo (esc_attr( BoldThemesFramework::$left_alignment_class )) ?>">
															<?php echo boldthemes_get_heading_html( $categories_html, $full_title, boldthemes_get_the_excerpt( get_the_ID() ), 'extralarge', $dash, 'bold', '' ) ;; ?>
														</div><!-- /boldBlogArticle -->
								...
								<div class="btInfoBar">
									<div class="btInfoBarMeta">
										...
										<p><strong><?php esc_html_e( 'Title:', 'fast-food' ); ?></strong> <span class="btPortfolioSliderCaption"></span></p>
										<p><strong><?php esc_html_e( 'Description:', 'fast-food' ); ?></strong> <span class="btPortfolioSliderDescription"></span></p>
									</div><!-- /boldInfoBarMeta -->
								</div><!-- /boldInfoBar -->
							</div><!-- /rowItemContent -->
						</div><!-- /rowItem -->
	...
	$content_html = str_replace( ']]>', ']]&gt;', $content_html );
		...
		if ( '' != $prev ) {
			...
			$prev_next_html .= '<span class="nbsItem"><span class="nbsDir">' . esc_html__( 'previous', 'fast-food' ) . '</span><span class="nbsTitle">' . esc_html( $prev->post_title ) . '</span></span>';
			$prev_next_html .= '</a></h4>';
		}
		$prev_next_html .= '</div>';
		...
		if ( '' != $next ) {
			...
			$url = $thumb[0];
			$prev_next_html .= '<span class="nbsItem"><span class="nbsDir">' . esc_html__( 'next', 'fast-food' ) . '</span><span class="nbsTitle">' . esc_html( $next->post_title ) . '</span></span>';
			$style_el = '';
