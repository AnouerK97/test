			...
			echo '<div class="boldCell">';
				echo '<div class="boldRow bottomSmallSpaced">';
					...
					if( boldthemes_get_option( 'hide_headline' ) ) {
						echo boldthemes_get_heading_html( '', $full_title . "</em>", boldthemes_get_the_excerpt( get_the_ID() ), 'large', $dash, 'wArticleMeta bold', '' ) ;
					}
					...
					if ( ( $cf != '' && count( $cf ) > $data_items_split ) || $categories_html != '' || $cf_right_html != '' ) {
						...
						if ( $categories_html != '' ) {
							echo '<dt>' . esc_html__( 'Category', 'fast-food' ) . '</dt>';
					...
					wp_link_pages( array( 
						'before'      => '<ul>' . esc_html__( 'Pages:', 'fast-food' ),
						'separator'   => '<li>',
						'after'       => '</ul>'
					));					
