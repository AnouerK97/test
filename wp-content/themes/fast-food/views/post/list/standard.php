<?php
	...
	$share_html = '<div class="btIconRow">' . boldthemes_get_share_html( $permalink, 'blog', 'btIcoSmallSize', 'btIcoDefaultType btIcoDefaultColor' ) . '</div>';
						...
						echo '<div class="rowItemContent ' . esc_attr( BoldThemesFramework::$left_alignment_class )  . '">';
							if ( $blog_side_info ) {
								...
								echo '<div class="btArticleListBody' . $extra_class . '">';				
									...
									echo boldthemes_get_heading_html( $categories_html, '<a href="' . esc_url_raw( $permalink ) . '">' . get_the_title() . '</a>', $meta_html, 'large bold', $dash, 'bold', '' );
									...
									echo '<div class="boldRow btArticleFooter">';
										echo '<div class="boldRowInner">';
											...
											echo '<div class="rowItem col-md-6 col-ms-12 btReadArticle btMiddleVertical ' . esc_attr( BoldThemesFramework::$right_alignment_class ) . '"><div class="rowItemContent">' . boldthemes_get_button_html( $continue_ico, $permalink, esc_html__( 'CONTINUE READING', 'fast-food' ), $continue_ico_style , '', '' ) . '</div></div>';
