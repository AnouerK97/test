		...
		echo '<div class="port">';
			echo '<div class="boldCell">';
				echo '<div class = "boldRow bottomSmallSpaced">';
					...
					if ( $meta_html != '' ) echo boldthemes_get_heading_html( $meta_html, '', '', 'large', '', 'bold', '' );
					if ( $blog_side_info ) {
					...
					echo '<div class="rowItem ' . esc_attr( BoldThemesFramework::$left_alignment_class )  . ' ' . $r_class . ' ">';
						echo '<div class="rowItemContent">';
							echo boldthemes_get_heading_html( $categories_html, '<a href="' . esc_url_raw( $permalink ) . '">' . get_the_title() . '</a>', '', 'large', $dash, 'bold', '' );
						echo '</div>' ;
					echo '</div>';
