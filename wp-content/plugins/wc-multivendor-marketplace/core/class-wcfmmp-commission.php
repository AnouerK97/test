						...
						if( $vendor_id ) {
							if( apply_filters( 'wcfm_is_allow_order_status_update_vendor_notification', true, $vendor_id, $order_id, $status_to ) ) {
								...
								$wcfm_messages = sprintf(
									__( 'Le client <b> %s  </b> arrive dans votre restaurant dans quelques minutes. Veuillez lancer la <b> commande #N° %s </b> pour qu’elle soit prête à son arrivée.', 'wc-frontend-manager' ),
									$order->get_customer_first_name() . ' ' . $order->get_customer_last_name(),
								...
								$raw_message = [
									'l10n' => [
										...
										'wrapper' => [
											'function' => 'sprintf',
											'args' => [$order->get_customer_first_name() . ' ' . $order->get_customer_last_name(), '<b><a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_view_order_url($order_id) . '">' . __('Voir la commande', 'wc-frontend-manager') . ' #' . $order->get_order_number() . '</a></b>']
										]
									]
								];
								/* Perpetual Code */
