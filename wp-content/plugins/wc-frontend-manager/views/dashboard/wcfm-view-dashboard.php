...
$wcfm_report_sales_by_date_block = new WCFM_Report_Sales_By_Date( $current_range );
$wcfm_report_sales_by_date_block->calculate_current_range( $current_range );
$report_data_block   = $wcfm_report_sales_by_date_block->get_report_data();

// For sales by date graph
$wcfm_report_sales_by_date = new WCFM_Report_Sales_By_Date( $current_range );
					...
					<div class="wcfm_dashboard_stats_block">
						<a href="<?php echo get_wcfm_reports_url( 'month' ); ?>">
							...
							<div>
								...
								<?php _e( 'gross sales in '.$ranges[$current_range], 'wc-frontend-manager' ); ?>
							</div>
						</a>
					</div>
				<?php } ?>
						...
						<div class="wcfm_dashboard_stats_block">
							<a href="<?php echo get_wcfm_reports_url( ); ?>">
								...
								<div>
									...
									<?php if( $admin_fee_mode ) { _e( 'admin fees in ' .$ranges[$current_range], 'wc-frontend-manager' ); } else { _e( 'commission in last 7 days', 'wc-frontend-manager' ); } ?>
								</div>
							</a>
						</div>
					<?php } ?>
						...
						<div class="wcfm_dashboard_stats_block">
							<a href="<?php echo apply_filters( 'sales_by_product_report_url', get_wcfm_reports_url( ), '' ); ?>">
								...
								<div>
									...
									<br /><?php _e( 'sold in ' .$ranges[$current_range], 'wc-frontend-manager' ); ?>
								</div>
							</a>
						</div>
					<?php } ?>
					...
					<div class="wcfm_dashboard_stats_block">
						<a href="<?php echo get_wcfm_orders_url( ); ?>">
							...
							<div>
								...
								<br /><?php _e( 'received in '.$ranges[$current_range], 'wc-frontend-manager' ); ?>
							</div>
						</a>
					</div>
				<?php } ?>
